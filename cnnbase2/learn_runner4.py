...
if __name__ == '__main__':
    ...
    epoch_pack = 15
    init_epoch = 60
    model2.learn_now(init_epoch)
    model2.save_model_to_file(filename + ('.e60.manual.save.') + model2.last_history_timestamp)
    model2.save_model_to_file(filename + ('.e60.') + model2.last_history_timestamp)
    for ei in range(6):
